...
import { MoreVertical, Package, PackageOpen, CheckCircle2, Trash2, ChevronDown, Zap } from 'lucide-react';
    ...
    >
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="flex items-center gap-2 mt-1 flex-wrap">
            ...
            {item.shelf && (
              <>
                ...
                <span className="text-xs text-slate-400">{item.shelf}</span>
              </>
            )}
            <span className="text-slate-300">·</span>
          ...
          {item.notes && (
            <p className="text-xs text-slate-400 mt-1.5 truncate italic">{item.notes}</p>
          )}
        ...
        <div className="relative flex-shrink-0">
          <button
            ...
            className={cn(
              ...
              showActions && 'bg-white/60 text-slate-600',
            )}
          >
          ...
          {showActions && (
            <>
              ...
              <div className="absolute right-0 top-9 z-20 w-52 glass-heavy rounded-2xl shadow-glass-hover py-1.5 animate-scale-in">
                <button
                  onClick={() => { onToggleOpened(item.id); setShowActions(false); }}
                  className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-white/50 flex items-center gap-2.5 transition-colors"
                >
                ...
                <button
                  onClick={() => { onMarkConsumed(item.id); setShowActions(false); }}
                  className="w-full text-left px-4 py-2.5 text-sm text-slate-700 hover:bg-white/50 flex items-center gap-2.5 transition-colors"
                >
                  <CheckCircle2 className="w-4 h-4 text-fresh-500" />
                  Used It Up
                </button>
                <div className="my-1 mx-3 h-px bg-slate-200/60" />
                <button
                  onClick={() => { onMarkWasted(item.id, 'expired'); setShowActions(false); }}
                  className="w-full text-left px-4 py-2.5 text-sm text-danger-600 hover:bg-danger-50/60 flex items-center gap-2.5 transition-colors"
                >
                  <span className="text-sm">🗑️</span> Wasted (Expired)
                </button>
                <button
                  onClick={() => { onMarkWasted(item.id, 'spoiled'); setShowActions(false); }}
                  className="w-full text-left px-4 py-2.5 text-sm text-danger-600 hover:bg-danger-50/60 flex items-center gap-2.5 transition-colors"
                >
                ...
                <button
                  ...
                  className="w-full text-left px-4 py-2.5 text-sm text-slate-500 hover:bg-slate-100/50 flex items-center gap-2.5 transition-colors"
                >
      ...
      {hasNutrition && showNutrition && (
        <div className="px-4 pb-4 border-t border-white/20 pt-3 animate-fade-in">
