...
const typeConfig: Record<ToastType, { icon: React.ReactNode; classes: string }> = {
  success: {
    icon: <CheckCircle className="w-4 h-4 text-fresh-600" />,
    classes: 'border-l-4 border-fresh-400 bg-fresh-50/90',
  },
  error: {
    icon: <AlertCircle className="w-4 h-4 text-danger-600" />,
    classes: 'border-l-4 border-danger-400 bg-danger-50/90',
  },
  warning: {
    icon: <AlertTriangle className="w-4 h-4 text-warning-600" />,
    classes: 'border-l-4 border-warning-400 bg-warning-50/90',
  },
  info: {
    icon: <Info className="w-4 h-4 text-frost-600" />,
    classes: 'border-l-4 border-frost-400 bg-frost-50/90',
  },
};
...
function ToastCard({ item, onRemove }: { item: ToastItem; onRemove: (id: string) => void }) {
  ...
  return (
    <div
      className={cn(
        'flex items-start gap-3 px-4 py-3 rounded-2xl shadow-glass backdrop-blur-glass max-w-xs w-full',
